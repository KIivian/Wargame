...
namespace Wargame
{
    public partial class PlacementForm : Form
    {
        ...
        private void CheckMouse()
        {
            ...
            {
                var distance = Geometry.GetDistance(mousePos, ShipIcoCenters[i]);
        ...
        private void SetInfoLabelText()
        {
            ...
            {
                case Battleship _:
                    InfoLabel.Text = "Линкор\n";
                    break;
                case Carrier _:
                    InfoLabel.Text = "Авианосец\n";
                    break;
                case HeavyCruiser _:
                    InfoLabel.Text = "Тяжелый крейсер\n";
                    break;
                case Cruiser _:
                    InfoLabel.Text = "Крейсер\n";
                    break;
                case Fregate _:
                    InfoLabel.Text = "Фрегат\n";
                    break;
                case Gunship _:
                    InfoLabel.Text = "Канонерка\n";
                    break;
                case Destroyer _:
                    InfoLabel.Text = "Эсминец\n";
                    break;
                case Freighter _:
                    InfoLabel.Text = "Фрейтер\n";
                    break;
                case Transport _:
                    InfoLabel.Text = "Транспорт\n";
                    break;
                case Fighter _:
                    InfoLabel.Text = "Истребитель\n";
                    break;
                case DoubleFighter _:
                    InfoLabel.Text = "Два истребителя\n";
                    break;

            }
            InfoLabel.Text +=
               "Цена: " + SelectedShip.Price + "\n\n" +
               "Щиты: " + SelectedShip.Shields + "\n" +
               "Скорость: " + SelectedShip.Speed + "\n" +
               "Уклонение: " + SelectedShip.Evasion + "\n" +
               "Орудия: ";
            ...
            switch (SelectedShip)
            {
                ...
                case Carrier _:
                    InfoLabel.Text += "Огромный корабль, немногим меньше космостанции. По сути скорее база для управления меньшими кораблями, чем самостоятельная боевая единица." + "\n"
                      + "Авианосец несет на себе 12 истребителей и может вместо своего хода выпустить до шести из них на соседние гексы." + "\n";
                    return;
                ...
                case Gunship _:
                    InfoLabel.Text += "Средний по размерам корабль, на котором скорость и живучесть принесена в жертву огневой мощи." + "\n"
                        + "Дальнобойные турболазеры позволяли быстро нанести большой урон, не входя в зону поражения орудий противника. Во время своего хода вместо движения может выстрелить второй раз.";
                    return;
        ...
        {
            if (Main.FleetPoints[Main.Player] == 0)
            {
                MoneyLabel.Text = "НЕТ ОЧКОВ";
                MoneyLabel.ForeColor = Color.Red;
            }
            else
            {
                MoneyLabel.Text = "ОЧКИ: " + Main.FleetPoints[Main.Player];
            }
            MoneyLabel.Refresh();
        }
        ...
        {
            var corners = Geometry.GetHexCorners(center, Main.HexRadius);
